...
public:  
  ...
  void basisFunsU(T u, int span, Vector<T>& vec) const ;
  void basisFunsV(T u, int span, Vector<T>& vec) const ;
  ...
  // Point projection on surface functions
  bool projectTo(const Point_nD<T,N>& p, T guess_u, T guess_v, T& u, T& v,
    Point_nD<T,N>& r, T e1, T e2,int& maxTry) const;
//  bool projectTo(const Point_nD<T,N>& p, T guess_u, T guess_v, T& u, T& v,
//    Point_nD<T,N>& r, T e1=0.001, T e2=0.001,int maxTry=100) const;
  bool projectTo(const Point_nD<T,N>& p, T guess_u, T guess_v, T& u, T& v,
    Point_nD<T,N>& r, T umin, T umax, T vmin, T vmax, 
    T e1=0.001, T e2=0.001,int maxTry=100) const;
  bool projectToUsingModifiedNewton(const Point_nD<T,N>& p, T guess_u, T guess_v, T& u, T& v,
    Point_nD<T,N>& r, T e1, T e2, int maxTry) const;
  bool projectToU(const Point_nD<T,N>& p, T guess_u, T fixed_v, T& u,
    Point_nD<T,N>& r, T e1, T e2, int maxTry=100) const;
  bool projectToV(const Point_nD<T,N>& p, T fixed_u, T guess_v, T& v,
    Point_nD<T,N>& r, T e1, T e2, int maxTry=100) const;
  ...
  void makeFromRevolution(const NurbsCurve<T,N>& profile, const Point_nD<T,N>& S, const Point_nD<T,N>& Tvec, double theta) ;
  void makeFromRevolution(const NurbsCurve<T,N>& profile, const Point_nD<T,N>& S, const Point_nD<T,N>& Tvec) ;
  ...
  int decompose(NurbsSurfaceArray<T,N>& Sa) const ; // Decompose the surface into Bézier strips
  int decomposeV(NurbsSurfaceArray<T,N>& Sa) const ; // Decompose the surface into Bézier strips in v direction
  ...
  int writeVTU(const char* filename, T step_u, T step_v);
